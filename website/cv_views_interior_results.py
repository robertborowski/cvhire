...
def results_none_function():
  return redirect(url_for('cv_views_interior_results.cv_dashboard_general_function', url_status_code='active'))
...
@cv_views_interior_results.route('/results/<url_status_code>/<url_redirect_code>/', methods=['GET', 'POST'])
@login_required
def results_dashboard_general_function(url_status_code='active', url_redirect_code=None):
  ...
  if url_status_code not in status_codes_arr:
    return redirect(url_for('cv_views_interior_results.cv_dashboard_general_function', url_status_code='active', url_redirect_code='e10'))
  ...
  page_dict['dashboard_action_link'] = '/results/add'
  ...
  if updates_occurred == True:
    return redirect(url_for('cv_views_interior_results.cv_dashboard_general_function', url_status_code='active', url_redirect_code=url_redirect_code))
  # ------------------------ autofill newly added cv/resumes end ------------------------
  # ------------------------ choose correct template start ------------------------
  correct_template = ''
  if url_status_code == 'active':
    correct_template = 'interior/results/active/index.html'
  if url_status_code == 'archive':
    correct_template = 'interior/results/archive_cv/index.html'
  if url_status_code == 'all':
    correct_template = 'interior/results/all_cv/index.html'
